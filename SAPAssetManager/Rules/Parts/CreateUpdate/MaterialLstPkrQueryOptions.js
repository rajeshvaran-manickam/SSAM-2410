...
export default function MaterialLstPkrQueryOptions(context) {
    try {
        ...
        if (onlineSwitchValue && isDefenseEnabled(context)) {
            newFilterOpts = AddDefenseOnlineFilters(context, newFilterOpts);
        }
        ...
        if (context.searchString) {
            if (onlineSwitchValue) {
                ...
                if (isDefenseEnabled(context)) {
                    filters.push(`substringof('${searchString}', Material/NATOStockNum) eq true`);
                    filters.push(`substringof('${searchString}', Material/LongManufacturerPartNum) eq true`);
                    filters.push(`substringof('${searchString}', Material/ManufacturerPartNum) eq true`);
                }
                materialLstPkrQueryOptions.filter(`(${filters.join(' or ')})`);
            }
        }
...
function AddDefenseOnlineFilters(context, newFilterOpts) {
    const pageProxy = context.getPageProxy();
    const filters = [...newFilterOpts];

    const NATOStockNum = pageProxy.evaluateTargetPath('#Control:MaterialNatoStockNumber')?.getValue();
    const LongManufacturerPartNum = pageProxy.evaluateTargetPath('#Control:MaterialMPNStockNumber')?.getValue();
    const ManufacturerPartNum = pageProxy.evaluateTargetPath('#Control:MaterialLMPNStockNumber')?.getValue();

    if (NATOStockNum) {
        filters.push(`substringof('${NATOStockNum}', NATOStockNum)`);
    }

    if (LongManufacturerPartNum) {
        filters.push(`substringof('${LongManufacturerPartNum}', LongManufacturerPartNum)`);
    }

    if (ManufacturerPartNum) {
        filters.push(`substringof('${ManufacturerPartNum}', ManufacturerPartNum)`);
    }
