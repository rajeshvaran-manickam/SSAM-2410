...
export default class {
    ...
    static setStartStatus(context, binding) {
        ...
        libThis.setMobileStatus(binding || libCommon.getBindingObject(context), clientData.ChangeStatus, libCommon.getUserGuid(context), libCommon.getSapUserName(context));
    }
    ...
    static setHoldStatus(context) {
        ...
        libThis.setMobileStatus(libCommon.getBindingObject(context), clientData.ChangeStatus, libCommon.getUserGuid(context), libCommon.getSapUserName(context));
    }
    ...
    static setTransferStatus(context) {
        ...
        libThis.setMobileStatus(context.binding, clientData.ChangeStatus, libCommon.getUserGuid(context), libCommon.getSapUserName(context));
    }

    static setCompleteStatus(context, binding) {
        let clientData = context.getClientData();
    ...
    static phaseModelStatusChange(context, status) {
        switch (status) {
            case 'COMPLETED':
                return libThis.setCompleteStatus(context);
            case 'HOLD':
                return libThis.setHoldStatus(context);
            case 'REJECTED':
                return libThis.setRejectedStatus(context);
            case 'REVIEW':
                return libThis.setReviewStatus(context);
            case 'STARTED':
                return libThis.setStartStatus(context);
            case 'SUCCESS':
                return libThis.setSuccessStatus(context);
            case 'TRANSFER':
                return libThis.setTransferStatus(context);
            case 'DISAPPROVE':
                return libThis.setDisapprovedStatus(context);
            case 'APPROVE':
                return libThis.setApprovedStatus(context);
            default:
                return libThis.setGenericStatus(context, status);
        }
    }
    ...
    /**
     * Gets mobile status object for specific data type
     * @param {IClientAPI} context 
     * @param {*} binding 
     * @returns {PMMobileStatus}
     */
    static getMobileStatusNavLink(context, binding = context.binding) {
        if (binding) {
            switch (binding['@odata.type']) {
                case '#sap_mobile.MyWorkOrderHeader':
                ...
                case '#sap_mobile.MyWorkOrderSubOperation':
                        return binding.SubOpMobileStatus_Nav;
                case '#sap_mobile.MyNotificationHeader':
                    return binding.NotifMobileStatus_Nav;
                case '#sap_mobile.MyNotificationTask':
                    return binding.TaskMobileStatus_Nav;
                case '#sap_mobile.MyNotificationItemTask':
                    return binding.ItemTaskMobileStatus_Nav;
                case '#sap_mobile.S4ServiceItem':
                case '#sap_mobile.S4ServiceOrder':
                case '#sap_mobile.S4ServiceRequest':
                case '#sap_mobile.S4ServiceConfirmation':
                case '#sap_mobile.S4ServiceConfirmationItem':
                    return binding.MobileStatus_Nav;
                default:
                    return binding.MobileStatus;
            }
        }
        return '';
    }
    ...
    static async isMobileStatusConfirmed(context, actionBinding, SubOperation = '') {
        ...
        if (orderId && operation) {
            ...
            return !libEval.evalIsEmpty(confirmations) && confirmations.getItem(0)?.FinalConfirmation === 'X';
        } else {
    ...
    static checkConfirmationsForFinalState(confirmations) {
        if (libEval.evalIsEmpty(confirmations)) return false;
    ...
    static rollupCompleteStatusToChildPages(context, parent, review) {
        ...
        switch (parent['@odata.type']) {
            case '#sap_mobile.MyWorkOrderHeader': {
                ...
                page = `#Page:${WorkOrderOperationDetailsPageNameToOpen(context)}`;
                pageContext = '';
                try {
                    pageContext = context.evaluateTargetPathForAPI(page);
                } catch (err) {
                ...
                page = `#Page:${SubOperationDetailsPageName(context)}`;
                pageContext = '';
                try {
                    pageContext = context.evaluateTargetPathForAPI(page);
                } catch (err) {
    ...
    */
    static getMobileStatusValueConstants(context) {
        return {
            ...
            WORKCOMPL: context.getGlobalDefinition('/SAPAssetManager/Globals/MobileStatus/ParameterNames/WorkCompletedParameterName.global').getValue(),
            ASSIGN: context.getGlobalDefinition('/SAPAssetManager/Globals/MobileStatus/ParameterNames/Supervisor/AssignStatusValue.global').getValue(),
            UNASSIGN: context.getGlobalDefinition('/SAPAssetManager/Globals/MobileStatus/ParameterNames/Supervisor/UnAssignStatusValue.global').getValue(),
            REASSIGN: context.getGlobalDefinition('/SAPAssetManager/Globals/MobileStatus/ParameterNames/Supervisor/ReAssignStatusValue.global').getValue(),
            CONFIRM: context.getGlobalDefinition('/SAPAssetManager/Globals/MobileStatus/ParameterNames/ConfirmUnconfirm/ConfirmStatusValue.global').getValue(),
            UNCONFIRM: context.getGlobalDefinition('/SAPAssetManager/Globals/MobileStatus/ParameterNames/ConfirmUnconfirm/UnconfirmStatusValue.global').getValue(),
        };
    }
    ...
    static getObjectTypeForCurrentAssignmentLevel(context) {
        if (IsS4ServiceIntegrationEnabled(context)) {
            switch (libCommon.getS4AssnTypeLevel(context)) {
                case 'Header':
                    return context.getGlobalDefinition('/SAPAssetManager/Globals/S4Service/OrderMobileStatusObjectType.global').getValue();
                case 'Item':
                    return context.getGlobalDefinition('/SAPAssetManager/Globals/S4Service/ItemMobileStatusObjectType.global').getValue();
                default:
                    return '';
            }
        }
        ...
        switch (libCommon.getWorkOrderAssnTypeLevel(context)) {
            case 'Header':
                return context.getGlobalDefinition('/SAPAssetManager/Globals/ObjectTypes/WorkOrder.global').getValue();
            case 'Operation':
            case 'SubOperation':
                return context.getGlobalDefinition('/SAPAssetManager/Globals/ObjectTypes/WorkOrderOperation.global').getValue();
            default:
                return '';
        }
    }
