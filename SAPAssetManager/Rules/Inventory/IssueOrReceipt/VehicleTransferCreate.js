...
export default function VehicleTransferCreate(context) {
    ...
    let valuationTypePickerValue = libCom.getListPickerValue(libCom.getControlProxy(context, 'ValuationTypePicker').getValue());
    ...
    if (materialPickerValue) {
        ...
        return context.read(service, materialPickerValue, [], query).then(result => {
            ...
            if (valuationTypePickerValue) {
                binding.TempLine_ValuationType = valuationTypePickerValue;
            }
            binding.TempLine_SpecialStockInd = '';
