 ...
 export class CreateUpdateFunctionalLocationEventLibrary {
    ...
    static onPageLoad(pageClientAPI) {
        if (!pageClientAPI.getClientData().LOADED) {
            ...
            if (!onCreate) {
                this.disableControlsOnEdit(pageClientAPI);
            }
     ...
     */
     static setValues(pageProxy, FLOC) {
        ...
        if (FLOC.ConstMonth && FLOC.ConstYear) {
            ...
            date.setMonth(FLOC.ConstMonth - 1);
    ...
    static validateValues(context, createFlag) {
        ...
        const isNotLocal = !IsOnCreate(context) && context.binding && !libCommon.isCurrentReadLinkLocal(context.binding['@odata.readLink']);
        if (isNotLocal) {
    ...
    static setSuperioir(context, superior) {
        ...
        if (value) {
            extension = formCellContainer.getControl('SuperiorFuncLocHierarchyExtensionControl').getExtension();
        }
        ...
        if (IsEditTechObjectFeatureEnabled(context) && !libCommon.IsOnCreate(context)) {
            setTimeout(() => {
                formCellContainer.getControl('SuperiorFuncLocHierarchyExtensionControl').getExtension().setEditable(false);
            }, 200);
        }
    }

    static disableControlsOnEdit(pageProxy) {
        const isNotLocal = !libCommon.isCurrentReadLinkLocal(pageProxy.binding['@odata.readLink']);
        if (isNotLocal) {
            libCommon.getControlProxy(pageProxy, 'CategoryLstPkr').setVisible(false);
            libCommon.getControlProxy(pageProxy, 'StrcutureIndLstPkr').setVisible(false);
        }
    }
}
