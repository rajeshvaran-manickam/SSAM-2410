...
export default class {
    ...
    */
     ...
     */
    static measurementDocumentCreateUpdateValidation(pageClientAPI, singleDoc = false, binding = null, dict = {}) {
        if (!binding) {
            dict = {};
        ...
        if (libVal.evalIsEmpty(dict.ValuationCodeLstPkr.getValue()) && libVal.evalIsEmpty(dict.ReadingSim.getValue())) {
            // if we are creating/editing a single measurement doc, then we can't skip, we should display validation
            if (singleDoc) {
                const message = pageClientAPI.localizeText('field_is_required');
    ...
    */
    static evalShortTextLengthWithinLimit(dict, limit) {
        return (dict.ShortTextNote.getValue()?.length || 0  <= Number(limit));
    }
