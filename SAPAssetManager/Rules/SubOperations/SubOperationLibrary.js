...
import { OperationEventLibrary } from '../WorkOrders/Operations/WorkOrderOperationLibrary';
import StandardTextPickerIsVisible from '../WorkOrders/Operations/CreateUpdate/StandardTextPickerIsVisible';
import WorkOrderOperationSelectedSTK from '../WorkOrders/Operations/WorkOrderOperationSelectedSTK';
...
export class SubOperationEventLibrary {
     ...
     * @param {IPageProxy} pageProxy
     */
    static async createUpdateOnCommit(pageProxy) {
        ...
        if (await StandardTextPickerIsVisible(pageProxy)) {
            extraProperties.StandardTextKey = WorkOrderOperationSelectedSTK(pageProxy);
        }

        if (libCommon.IsOnCreate(pageProxy)) {
            return pageProxy.executeAction({
                'Name': '/SAPAssetManager/Actions/WorkOrders/SubOperations/SubOperationCreate.action',
                'Properties': {
                    'Properties': extraProperties,
                },
            });
        } else {
            return OperationEventLibrary.addNoteAfterUpdate(pageProxy, {
                'Name': '/SAPAssetManager/Actions/WorkOrders/SubOperations/SubOperationUpdate.action',
                'Properties': {
                    'Properties': extraProperties,
                },
            });
...
export class PrivateMethodLibrary {
    ...
    static _getParentWorkOrder(context) {
        ...
        if (clientData.ParentWorkOrder) {
            return Promise.resolve(clientData.ParentWorkOrder);
        } else {
            let woReadLink = '';
            const onCreate = libCommon.IsOnCreate(context);
            const binding = context.binding;
            if (onCreate && binding?.['@odata.readLink']) {
                woReadLink = binding['@odata.readLink'] + '/WOHeader';
            } else if (binding?.WorkOrderOperation?.WOHeader?.['@odata.readLink']) {
                woReadLink = binding.WorkOrderOperation.WOHeader['@odata.readLink'];
            }

            if (woReadLink) {
                return context.read('/SAPAssetManager/Services/AssetManager.service', woReadLink, [], '')
                    .then(workOrder => {
                        clientData.ParentWorkOrder = workOrder.getItem(0);
