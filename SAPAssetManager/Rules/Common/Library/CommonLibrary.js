...
export default class {
    ...
    */
        ...
        try {
            ...
            if (fioriToolbar) {
                fioriToolbar.getItems()
                    ...
                    .forEach(item => item.setEnabled(flag));
            }
            if (pageToolbar) {
    ...
    static decodeReadLink(readLink) {
        let components = readLink.match(/([A-z]+=)?'(\d|[A-z]|%|[a-zA-Z0-9\-])+'/g);
    ...
    */
    ...
    static setBindingObject(context) {
        ...
        context.constructor.name === 'PageProxy' && context.getPageProxy().getExecutedContextMenuItem()) {
