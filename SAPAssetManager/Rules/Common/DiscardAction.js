        ...
        if (successResult.data) {
            ...
            if (erroPageExists) {
                ...
                switch (entityName) {
                    ...
                    case 'S4ServiceOrderPartner':
                        action = context.executeAction('/SAPAssetManager/Actions/BusinessPartners/S4/S4ServiceOrderPartnerDelete.action');
                        break;
                    case 'S4ServiceRequestPartner':
                        action = context.executeAction('/SAPAssetManager/Actions/BusinessPartners/S4/S4ServiceRequestPartnerDelete.action');
                        break;
                    case 'S4ServiceQuotation':
                        action = context.executeAction('/SAPAssetManager/Actions/ServiceQuotations/ServiceQuotationDelete.action');
                        break;
                    case 'S4ServiceQuotationItem':
                        action = context.executeAction('/SAPAssetManager/Actions/ServiceQuotations/Items/ServiceQuotationItemDelete.action');
                        break;
                    default:
                        break;
                }
 ...
 */
function PIDiscardItem(context) {
    const action = '/SAPAssetManager/Actions/Inventory/PhysicalInventory/PhysicalInventoryDocItemDiscard.action';
    return DiscardImpl(context, action);
}
 ...
 * @returns the result of the action execution
 */
function MDDiscardItem(context) {
    const action = '/SAPAssetManager/Actions/Inventory/MaterialDocumentItem/MaterialDocItemDelete.action';
    return DiscardImpl(context, action);
}
 ...
 * @returns 
 */
function DiscardImpl(context, action) {
    const readLink = context.binding['@odata.readLink'] || context.binding.item['@odata.readLink'];
