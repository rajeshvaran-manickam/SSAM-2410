...
export default class {
     ...
     */
    static reloadUserTimeEntries(context, returnEmpty = false, clockOut = undefined, businessObject = undefined, execute = true) {
        let binding;
        ...
        if (libThis.isBusinessObjectClockedIn(context, businessObject, businessObject.UserTimeEntry_Nav)) { //Handle clock out if this object is clocked in
            ...
            if (businessObject['@odata.type'] === '#sap_mobile.MyWorkOrderOperation') {
                ...
                return libThis.removeUserTimeEntries(context, query, true, false, true).then(() => {
                    ...
                    try {
                        operationContext = context.evaluateTargetPathForAPI(`#Page:${WorkOrderOperationDetailsPageNameToOpen(context)}`);
                    } catch (err) {
                        //page does not exist
                    }
                    if (query === 'SKIP') { //Time was not entered, so clock this object out
    ...
    static updateCICOOnStatusUpdate(context, updateToStatus, binding = context.binding) {
        const cicoValue = libThis.getCICOValue(context, updateToStatus);
        const userGUID = libCom.getUserGuid(context);
        const userId = libCom.getSapUserName(context);
        const linkProperties = libThis.getLinkProperties(context, binding['@odata.type']);
        ...
        if (cicoValue) { // Only add to CICO if status requires
            return context.executeAction({
                'Name': '/SAPAssetManager/Actions/ClockInClockOut/WorkOrderClockInOut.action', 'Properties': {
                    'Properties': {
    ...
    static getCICOValue(context, updateToStatus) {
        const cicoEnabled = libThis.isCICOEnabled(context) && libCom.getWorkOrderAssnTypeLevel(context) !== 'SubOperation';
        const { STARTED, HOLD } = libMobile.getMobileStatusValueConstants(context);

        // Create relevant CICO entry
        switch (updateToStatus) {
            case STARTED:
                return cicoEnabled ? 'CLOCK_IN' : 'START_TIME';
            case HOLD:
                return cicoEnabled ? 'CLOCK_OUT' : 'END_TIME';
            default:
                return '';
        }
    }

    static getLinkProperties(context, odataType) {
        switch (odataType) {
            case context.getGlobalDefinition('/SAPAssetManager/Globals/ODataTypes/WorkOrderOperation.global').getValue():
                return {
                    linkEntitySet: 'MyWorkOrderOperations',
                    linkProperty: 'WOOperation_Nav',
                };
            case context.getGlobalDefinition('/SAPAssetManager/Globals/ODataTypes/WorkOrderSubOperation.global').getValue():
                return {
                    linkEntitySet: 'MyWorkOrderSubOperations',
                    linkProperty: 'WOSubOperation_Nav',
                };
            default:
                return {
                    linkEntitySet: 'MyWorkOrderHeaders',
                    linkProperty: 'WOHeader_Nav',
                };
        }
    }
}
