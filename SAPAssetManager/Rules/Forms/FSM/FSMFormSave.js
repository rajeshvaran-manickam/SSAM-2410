            ...
            if (formNameValueMap.size || isSubmit) { //Either save is required because of change, or user is submitting form
                ...
                return values_file.readText().then(res => {
                    ...
                    for (const control of formNameValueMap.keys()) { // will only have values that needs update
                        ...
                        if (index !== -1 && formControls[key].Type !== 'attachmentPicker') { // Make sure we only add when the index is found
                        ...
                        } else if (index !== -1 && formControls[key].Type === 'attachmentPicker') { //attachmentPicker does not overwrite existing value, it appends the value
                            let objectIndex = res.indexOf('<object-attachment');
                            let xml = FSMSmartFormsLibrary.createXMLString(key, formControls, formNameValueMap);
                            res = res.substring(0, objectIndex - 1) + xml + res.substring(objectIndex, res.length);
                        } else if (index === -1 && formControls[key].Type === 'attachmentPicker') {
                            let xml = FSMSmartFormsLibrary.createXMLString(key, formControls, formNameValueMap);
                            res = res.replace('</values>', '') + '\n\t' + xml + '</values>';
                        } else {
                            let xml = FSMSmartFormsLibrary.createXMLString(key, formControls, formNameValueMap);
                            res = res.replace('</values>', '') + '\n\t' + xml + '\n' + '</values>';
                        }
